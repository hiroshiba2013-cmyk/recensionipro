/*
  # Fix search function: region filter for registered businesses

  ## Problem
  The search_all_business_locations function hard-codes region as empty string
  for registered_business_locations and registered_businesses, and uses
  `AND (search_region IS NULL)` which means registered businesses NEVER appear
  when a region filter is applied.

  ## Fix
  - Derive region from a province-to-region lookup (using unclaimed_business_locations
    as reference, since it has accurate province+region data)
  - Replace `AND (search_region IS NULL)` with proper region filtering that:
    - Shows results when no region filter is set
    - Filters by derived region when a region filter is set
*/
...
RETURNS TABLE(
  id uuid, name text, business_name text, category_id uuid,
  description text, address text, city text, province text, region text,
  postal_code text, phone text, email text, website text,
  business_hours text, latitude numeric, longitude numeric,
  location_type text, is_claimed boolean, is_verified boolean,
  business_id uuid, owner_id uuid, added_by uuid,
  added_by_family_member_id uuid, source text,
